...
def main():
    ...
    model_0 = FashionMNISTModelV2(input_shape=1, hidden_units=10, output_shape=len(class_names))
    logger.info(model_0)
...
def make_confusion_matrix(model, test_data):
    ...
    confmat_tens = confmat(preds=pred_labels_tensor, target=test_data.targets)
    # print(confmat_tens)
    fig, ax = plot_confusion_matrix(
        conf_mat=confmat_tens.numpy(), class_names=class_names, figsize=(10, 7)
    )
    plt.show()
