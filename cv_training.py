...
from cv_typing import TargetDict, TargetDictPureTensor, ImageTransform, BoxedImageTransform
...
class BoundingBoxDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, root: Path | str, transforms: Optional[BoxedImageTransform], img_extension: str = IMG_FILE_EXT):
        ...
        self.img_files = sorted((self.root / "images").glob(f"*{img_extension}"))[0:8]
        self.box_files = sorted((self.root / "labels").glob("*.txt"))[0:8]
